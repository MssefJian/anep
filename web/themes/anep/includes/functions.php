<?php
function anep_render_block($key) {
...
function anep_base_url(){
  global $base_url;
  $theme_path = \Drupal::service('extension.list.module')->getPath('anep');
  return $base_url . '/' . $theme_path . '/';
}

global $gva_node_index;
function anep_preprocess_node(&$variables) {
  global $gva_node_index;
...
function anep_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function anep_preprocess_breadcrumb(&$variables){
...
function anep_print_icon($file, $alt){
